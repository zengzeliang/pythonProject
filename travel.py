...
class Config(object):
    ...
    def __init__(self, dataset):
        self.train_path = './travel_data/train/train_new.txt'
        self.test_path = './travel_data/test/test_new.txt'
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Linear(config.num_filters * len(config.filter_size),
                            config.num_classes)  # 输入的最后一个维度，输出的最后一个维度 全连接层只改变数据的最后一个维度 由输入最后的一个维度转化为类别数
    ...
    def forward(self, x):
        ...
        encoder_out, pooled = self.bert(context, attention_mask=mask,
                                        return_dict=False)  # output_all_encoded_layers 是否将bert中每层(12层)的都输出，false只输出最后一层【128*768】

        out = encoder_out.unsqueeze(
            1)  # 增加一个维度，[batch_size,channel_num,pad_size,embedding_num]  ->  [batch_size,channel_num,pad_size,embedding_num]
        out = torch.cat([self.conv_and_pool(out, conv) for conv in self.convs], 1)
        out = self.fc(out)  # 128*11
        return out
...
def  train_model():
    ...
    for epoch in range(config.num_epoch):
        ...
        if epoch == 4 or epoch == 9 or epoch == 14:
            torch.save(model.state_dict(), "./results/travel-model/model" + str(epoch) + ".pkl")
    ...
    for step, (x, y) in enumerate(test_dataloader):
        with torch.no_grad():
            ...
            for j in range(len(y)):
                ...
                p.append(np.argmax(outputs))

    print(labels, " ", p)
    print(model_name + " macro precision: ", precision_score(labels, p, average='macro'))
    print(model_name + " micro precision ", precision_score(labels, p, average='micro'))
    print(model_name + " precision: ", precision_score(labels, p, average=None))
    print("=======================")
